...
const ErrorState = () => (
  <div className="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-destructive/20 rounded-2xl bg-destructive/5 max-w-3xl mx-auto">
    ...
    <p className="text-muted-foreground max-w-sm">
      We couldn't fetch your Playlist videos. Please check your connection and
      try again.
    </p>
  </div>
)
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-border rounded-2xl bg-accent/20">
      ...
      <h2 className="text-xl font-semibold mb-2">
        Your Playlist list is empty
      </h2>
      <p className="text-muted-foreground max-w-sm mb-6">
        Save videos to Playlist by clicking the clock icon on any video.
      </p>
      <button
        onClick={() => {
          navigate("/")
...
export const PlaylistPage = () => {
  ...
  const {
    ...
    queryFn: ({ pageParam }) =>
      getPlaylistDetails(playlistId!, { cursor: pageParam, limit: 20 }),
    getNextPageParam: (lastPage) => {
      const lastItem = lastPage.items.at(-1)
      return lastItem?.videoId
    },
  ...
  const allItems = useMemo(
    () => playlistDetails?.pages.flatMap((page) => page.items) ?? [],
    [playlistDetails]
  )
  const videoCount = playlistDetails?.pages[0]?.videoCount ?? allItems.length
            ...
            <div className="space-y-4">
              {allItems.map((item, index) => (
                <VideoCard
                  ...
                  index={index}
                  onClick={() =>
                    navigate(`/videos/${item.videoId}?list=${playlistId}`)
                  }
                  onChannelClick={(e) => {
                    e.stopPropagation()
