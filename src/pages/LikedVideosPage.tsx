    ...
    >
      ...
      <div className="flex flex-col flex-1 min-w-0 gap-1.5 py-1 pr-8 sm:pr-0">
        ...
        <div
          ...
          onClick={(e) => {
            e.stopPropagation()
            navigate(`/channels/${item.video.channel.id}`)
          }}
        >
...
const ErrorState = () => (
  <div className="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-destructive/20 rounded-2xl bg-destructive/5 max-w-3xl mx-auto">
    ...
    <p className="text-muted-foreground max-w-sm">
      We couldn't fetch your Liked Videos. Please check your connection and try
      again.
    </p>
  </div>
)

const EmptyLikedVideosState = () => {
  const navigate = useNavigate()
  return (
...
export const LikedVideosPage = () => {
  ...
  const {
    ...
    queryFn: ({ pageParam }) => getLikedVideos(20, pageParam),
  ...
  const videoCount = likedVideos?.pages[0]?.videoCount ?? allItems.length

  if (isLoading) return <LikedVideoSkeleton />
